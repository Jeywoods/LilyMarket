...
Аукционная площадка — маркетплейс для торговли с real-time ставками, тёмной темой и адаптивным интерфейсом.
...
Гибридный подход REST и SignalR
Команды (ставка, отмена) требуют чёткого ответа — 200 OK или 400 с причиной. REST даёт это из коробки. События (перебитая ставка, победитель) нужно пушить сразу всем клиентам — это задача SignalR.
Вариант Чистый SignalR отклонён: ошибки валидации через хаб ломают привычную семантику, JWT и тесты сложнее. Вариант чистый REST с polling отклонён: polling убивает батарею на мобильных и даёт заметную задержку в торгах
